<script>
  ...
  export let currentColor = '#eab308';
  ...
  const colorsList = ['#333333', '#ef4444', '#f97316', '#f59e0b', '#10b981', '#eab308', '#8b5cf6', '#ec4899', '#fafafa'];
...
<style>
  ...
  .modal-content {
    background: #18181b; border: 1px solid #252525; border-radius: 12px;
    padding: 1.5rem; width: 350px; box-shadow: 0 12px 32px rgba(0,0,0,0.5);
    display: flex; flex-direction: column; gap: 1rem;
  }
  h3 { margin: 0; color: #fafafa; font-size: 1.25rem; }
  
  .form-group { display: flex; flex-direction: column; gap: 0.5rem; }
  label { font-size: 0.8rem; font-weight: 600; color: #a3a3a3; }
  input { background: #000; border: 1px solid #333333; color: #fff; padding: 0.6rem; border-radius: 6px; outline: none; transition: border-color 0.2s; font-family: inherit; }
  input:focus { border-color: #eab308; }
  
  .icon-grid { display: grid; grid-template-columns: repeat(6, 1fr); gap: 0.4rem; max-height: 200px; overflow-y: auto; padding: 0.5rem; background: #000; border-radius: 6px; border: 1px solid #252525; }
  .icon-btn { background: #252525; border: 2px solid transparent; padding: 0.5rem; border-radius: 6px; cursor: pointer; transition: all 0.2s; display: flex; justify-content: center; align-items: center; }
  .icon-btn:hover { background: #333333; }
  .icon-btn.selected { background: rgba(234,179,8,0.2); border-color: #eab308; }
  ...
  .cancel { background: transparent; color: #a3a3a3; }
