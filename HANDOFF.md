...
### Phase 22: FOH Parameter & Musician View Sync Integrity
- **Problem 1**: While scribble strips synced correctly, FOH dynamics (fader, gate, EQ, comp) did not update. `readFlatOscNumber` hid the `$mixerState` reactivity from the Svelte 5 compiler, and EQ hydration in `$effect` mutated a shallow proxy without creating a new Svelte 4 reference, causing the child `<EqEditor>` component to stall. Additionally, the X-Air Edit's individual EQ band power toggle wasn't being requested from the mixer.
- **Solution 1**: Appended `/ch/01-16/eq/{band}/on` to `mixerSync.js`. Rewrote FOH `<ChannelStrip>` to use the inherently reactive `getBentoParam` function directly within Svelte's markup to explicitly bind dynamics values. Implemented a `delete bands._cloned` cycle in the EQ `$effect` block to break Svelte 4 object-identity caching, enforcing redraws.
- **Problem 2**: Channel fader levels set across Aux Buses natively in the X AIR Edit app did not propagate to the Musician View in OpenMix.
- **Solution 2**: Added the `^\/ch\/(\d{2})\/mix\/(\d{2})\/level$` OSC address match sequence into `App.svelte`'s main `$effect` untrack loop. It now intercepts X-Air Aux levels and surgically parses them into `newSends[in_${num}_aux${auxNum}]`, bringing full bidirectional sync parity to Musician mode.
